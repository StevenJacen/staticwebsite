    ...
    <nav className="sticky top-0 z-50 bg-white/95 shadow-sm backdrop-blur-sm">
      <div className="w-full px-5 sm:px-8">
        <div className="flex h-20 items-center gap-8">
          <button className="flex flex-shrink-0 items-center" onClick={goHome} aria-label="HeartMemo 首页">
          ...
          <div className="hidden items-center gap-7 lg:flex">
            <div className="group relative">
              ...
              <div className="invisible absolute left-0 top-full w-48 translate-y-3 rounded-2xl bg-white p-3 opacity-0 shadow-xl ring-1 ring-slate-100 transition group-hover:visible group-hover:translate-y-2 group-hover:opacity-100">
                {productLinks.map((link) => (
                  <button
                    key={link.href}
                    onClick={() => goTo(link.href)}
                    className="block w-full rounded-xl px-4 py-3 text-left text-sm font-bold text-slate-600 transition hover:bg-blue-50 hover:text-blue-600"
                  >
                    {link.label}
                  </button>
                ))}
            ...
            <div className="group relative">
              <button
                onClick={() => goTo('/about')}
                className={`flex items-center gap-1 text-base font-bold tracking-widest transition-colors hover:text-slate-950 ${
                  ['/about', '/support', '/cooperation'].includes(location.pathname) ? 'text-slate-950' : 'text-slate-500'
                }`}
              >
                关于我们
                <ChevronDown className="h-4 w-4" />
      ...
      {isOpen && (
        <div className="absolute left-0 w-full border-t border-slate-100 bg-white shadow-lg lg:hidden">
          <div className="space-y-2 px-6 py-5">
            <button
              onClick={() => goTo('/products')}
              className="block w-full rounded-xl px-3 py-2 text-left text-sm font-black text-slate-900"
            >
              产品
            </button>
            {productLinks.map((link) => (
              <button
                key={link.href}
                onClick={() => goTo(link.href)}
                className="block w-full rounded-xl px-6 py-2 text-left text-sm font-bold text-slate-500 hover:bg-blue-50 hover:text-blue-600"
              >
                {link.label}
              </button>
            ))}
            {navLinks.map((link) => (
              <button
                key={link.href}
                onClick={() => goTo(link.href)}
                className="block w-full rounded-xl px-3 py-2 text-left text-sm font-bold text-slate-600 hover:bg-slate-50 hover:text-slate-900"
              >
                {link.label}
              </button>
            ))}
